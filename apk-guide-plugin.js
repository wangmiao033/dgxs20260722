(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:620px){.apk-page-head{padding:18px}.apk-page-head h1{font-size:23px}.apk-stats{grid-template-columns:repeat(2,1fr)}.apk-grid{grid-template-columns:1fr}.apk-tools{align-items:stretch;flex-direction:column}.apk-source-grid{grid-template-columns:repeat(2,1fr)}.apk-hero-callout{align-items:flex-start;flex-direction:column}.apk-detail-head{padding:17px}.apk-detail-head h1{font-size:25px}}
    `;
  ...
  function heroCards(list) {
    return `<div class="apk-grid">${list.map(h=>`<a class="apk-card" href="#apk/hero/${h.id}"><div class="apk-card-top"><span class="apk-tag">${esc(h.class)}</span><span class="apk-tag">ID ${h.id}</span><span class="apk-quality">${qualityName[h.quality]||('品质'+h.quality)}</span></div><h3>${esc(h.name)}${h.subtitle?` <small>· ${esc(h.subtitle)}</small>`:''}</h3><p>${esc(h.position || h.desc || '游戏内英雄档案')}</p><div class="apk-attrs"><span>力量<b>${h.attrs.force}</b></span><span>智力<b>${h.attrs.wisdom}</b></span><span>魅力<b>${h.attrs.charm}</b></span><span>统率<b>${h.attrs.leadership}</b></span></div></a>`).join('')}</div>`;
  }
  ...
  function heroDetail(id) {
    ...
    main().innerHTML=`<div class="apk-detail"><header class="apk-detail-head"><span class="apk-source-badge">英雄 ID ${h.id} · ${esc(h.class)}</span><h1>${esc(h.name)}${h.subtitle?` <small>· ${esc(h.subtitle)}</small>`:''}</h1><p>${esc(h.position||h.desc)}</p><div class="apk-attrs"><span>力量<b>${h.attrs.force}</b></span><span>智力<b>${h.attrs.wisdom}</b></span><span>魅力<b>${h.attrs.charm}</b></span><span>统率<b>${h.attrs.leadership}</b></span></div></header><div class="apk-detail-body">
